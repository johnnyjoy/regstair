...
**Document status:** Authoritative implementation PRD  
...
**Input artifact:** `docs/input/NEXT_LEVEL_SERVICE_PRD_SUBMITTED.md`  
**Governing decisions:** [ADR index](decisions/README.md) and [threat model](THREAT_MODEL.md)
...
3. A deployment administrator can configure in YAML which registries are available for user credentials.
...
## 4.1 First-run control-plane setup

An empty deployment presents a dedicated first-run web setup, not the legacy operational dashboard. The workflow creates the first local administrator and enters the authenticated control plane immediately. The same one-shot operation is available through a JSON API for headless deployment automation. Until setup completes, operational and account APIs are closed.

Regstair binds to loopback by default and must be initialized over a trusted connection before an operator explicitly exposes it to an enterprise network. There is no default or generated administrator password. Host-only recovery remains available after setup and is not exposed as a pre-authentication web operation.
...
For a credential-capable source, challenge-first access is the default: Regstair attempts the selected OCI operation anonymously and retrieves the configured shared or current-user credential only if that same source returns an authentication challenge. A source may explicitly require preemptive credentials when necessary. Authentication or authorization failure never changes the selected route or advances fallback.
...
# 7. Docker Client Authentication

Docker-facing authentication is part of the local-account foundation because Regstair must identify the local user before selecting that user's upstream credential.

As established by ADR-0001, `docker login` uses:

- the local Regstair username; and
- a separately issued, time-limited Regstair access token in the password field.
...
- the raw token is generated securely and shown only when created;
- Regstair stores only a secure token hash and lifecycle metadata;
- a token can be revoked before expiration;
- disabling a user invalidates that user's Docker access;
...
Routes and registry/source definitions remain YAML-owned for this phase, as established by ADR-0002. The database does not contain a second mutable copy of registry configuration.

Each configured source defines whether and how Regstair may use it and whether users may attach credentials to it.

## 8.1 Configured source fields
...
The deployment administrator configures in YAML:
...
## 16.4 Registry visibility

An administrator can inspect:

- configured approved registries;
- enabled state and supported operations;
- whether user credentials are allowed;
- the verification repository;
- health;
- which routes use each registry.

Registry configuration is read-only in the UI for this phase. Deployment administrators make registry and route changes in the authoritative YAML configuration.
...
## Administrator registry visibility
...
## Slice 1 — Local account and Docker identity foundation
...
- time-limited Docker token issuance, hashing, expiration, and revocation;
- normal `docker login` authentication as the local user;
- anonymous OCI behavior preserved.

## Slice 2 — Approved registry projection

- source configuration fields for user-credential approval;
...
## Slice 4 — Runtime user-credential selection

- authenticated local user identity in pull and push resolution;
- explicit anonymous, shared, or current-user credential mode;
- private pull and push using the correct verified credential;
- real user-attributed Harbor push;
- authorization before cached private content or upstream credential use;
- failure semantics and request attribution.
...
- read-only approved-registry visibility;
- user administration;
- request detail;
...
3. A user can sign into the Regstair web interface and issue a time-limited Docker token.
4. A deployment administrator can expose Harbor as an approved credential target through YAML configuration.
...
11. A normal Docker login and push use the authenticated local user's verified credential for a real configured destination.
