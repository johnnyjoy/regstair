...
func main() {
	...
	flag.StringVar(&options.ListenAddr, "listen", ":80", "HTTP redirect and health-check listen address")
	flag.StringVar(&options.HTTPSListenAddr, "https-listen", ":443", "HTTPS application and OCI listen address; empty disables HTTPS")
	...
	flag.StringVar(&options.TLSCertFile, "tls-cert-file", os.Getenv("REGSTAIR_TLS_CERT_FILE"), "operator-supplied TLS server certificate")
	flag.StringVar(&options.TLSKeyFile, "tls-key-file", os.Getenv("REGSTAIR_TLS_KEY_FILE"), "operator-supplied TLS server private key")
	flag.StringVar(&options.TLSCAFile, "tls-ca-file", os.Getenv("REGSTAIR_TLS_CA_FILE"), "CA certificate offered to Regstair clients")
